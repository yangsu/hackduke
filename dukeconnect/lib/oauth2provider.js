...
OAuth2Provider.prototype.login = function() {
  ...
  return function(req, res, next) {
    ...
    try {
      ...
      extra_data = "schedule"; //data[3];
    } catch(e) {
      res.writeHead(400);
      return res.end(e.message);
    }

    console.log("info avaiable yoj: "+user_id+" "+extra_data);
    ...
    if(req.method == 'GET' && '/oauth/authorize' == uri) {
      ...
      if('allow' in req.body) {
        if('token' == response_type) {
          ...
          self.emit('create_access_token', user_id, client_id, perms, function(extra_data) {
          ...
          self.emit('save_grant', req, client_id, code, perms, function() {
            var extras = {
              code: code,
            };
